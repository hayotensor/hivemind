...
class RoutingTable:
    ...
    def add_or_update_node(self, node_id: DHTID, peer_id: PeerID) -> Optional[Tuple[DHTID, PeerID]]:
        """
        The following is updated to a proof of stake mechanism

        If model is live on-chain:
            Proof of stake must be validated before adding to routing tables
        Else:
            We wait until model is live on-chain to constantly recheck the proof of stake
